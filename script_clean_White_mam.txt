...
tree <- read.tree("mamm.tre")

#set strings as factors to false to avoid problems later

traits <- read.csv("Whiteetal_mamm.csv", stringsAsFactors=FALSE)

names(traits) <- c("group","species","mass","temp","MR","mean.mass","q10smr","ref")
...
use <- read.csv("White_mamm_clean1.csv", stringsAsFactors=FALSE)
...
write.csv(traits, "White_Mamm.csv", row.names=FALSE)
