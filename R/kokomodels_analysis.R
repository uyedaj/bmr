...
#args <- list("kolokotrones", "10000", "_rj_test8", "sumpars_koko_nq", "NN01")
...
mymcmc <- bayou.makeMCMC(cache$phy, cache$dat, pred=cache$pred, SE=0, model=model, prior=prior, startpar=startpar, new.dir=paste("../output/runs/f",args[[1]],sep=""), outname=paste(args[[1]],args[[3]],sep=""), plot.freq=NULL, ticker.freq=1000, samp = 100)
mymcmc$run(args[[2]])

chain <- mymcmc$load()
chain <- set.burnin(chain, 0.4)
saveRDS(chain, file=paste("../output/runs/f",args[[1]], "/", args[[1]], args[[3]],"_chain.rds",sep=""))
saveRDS(mymcmc, file=paste("../output/runs/f",args[[1]], "/", args[[1]], args[[3]],"_mcmc.rds",sep=""))
...
plot(ss)
print(ss$lnr)
saveRDS(ss, file=paste("../output/runs/f",args[[1]],"/",args[[1]],"_",args[[3]],".ss.rds",sep=""))
