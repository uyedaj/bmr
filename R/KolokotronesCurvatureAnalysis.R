...
#rm(list=ls(all=TRUE))
#setwd("~/repos/bmr/R/")
#args <- list("tetrapods_ei", 50000, "_mammcurve1")
...
## Quadratic no phylogeny
ols <- lm(lnBMR~lnMass + I(lnMass^2), data=dat)
ols0 <- lm(lnBMR ~ lnMass, data=dat)
...
tdkoko <- make.treedata(fulltree, merged)
tdkoko <- select(tdkoko, lnMass.k, lnBMR.k, lnMass, lnBMR, lnMass.pan)


ols <- lm(lnBMR.k ~ lnMass.k + I(lnMass.k^2), data=tdkoko$dat)
summary(ols)

ols2 <- lm(lnBMR.k ~ lnMass + I(lnMass^2), data=tdkoko$dat)
summary(ols2)

ols3 <- lm(lnBMR.k ~ lnMass.k + I(lnMass.k^2), data=tdkoko$dat)
summary(ols3)

pdf('../output/figures/KokovsWhite.pdf', width=11, height=7)
par(mfrow=c(1,2))
plot(tdkoko$dat$lnMass.k, tdkoko$dat$lnBMR.k, pch=21, bg=is.na(tdkoko$dat$lnMass)+1, cex=0.8, xlab="lnMass", ylab="lnBMR")
plot(tdkoko$dat$lnBMR.k, tdkoko$dat$lnBMR, xlab="log(BMR Koko kJ/h)", ylab="log(BMR White dataset)")
dev.off()
