...
rr <- c("pruned_r1", "pruned_r2", "pruned_r3", "pruned_r4", "pruned_r5", "pruned_r6")
...
mymcmc_gs <- readRDS("../output/runs/tetrapods_gs/tetrapods_gs_pruned_r1.mcmc.rds")
tree <- mymcmc_gs$tree
...
chain_gs <- combine.chains(chain_gs1, chain_gs2, burnin.prop=0)
chain_gs <- lapply(1:length(chain_gs), function(x) c(chain_gs[[x]], chain_gs3[[x]]))
#chain_gs <- combine.chains(chain_gs, chain_gs3, burnin.prop=0)
names(chain_gs) <- names(chain_gs1)
chain_gs$gen <- 1:length(chain_gs$gen)
attributes(chain_gs)$model <- attributes(chain1)$model; attributes(chain_gs)$model.pars <- attributes(chain1)$model.pars; attributes(chain_gs)$tree <- attributes(chain1)$tree; attributes(chain_gs)$dat <- attributes(chain1)$dat; attributes(chain_gs)$class <- attributes(chain1)$class; attributes(chain_gs)$burnin <-0
sumstats <- summary(chain_gs)
plot(chain_gs)

plot(unlist(chain_gs$beta2))
plot(unlist(sapply(chain_gs$beta2, function(x) x[1])), type="n", xlab="Generation", ylab="Quadratic Slope", main="Body mass curvature at the root")
lines(unlist(sapply(chain_gs$beta2, function(x) x[1])), col="red", lwd=2)
...
sb <- sumpars$sb
cladesummaries <- lapply(c(0, sb), function(x) summarizeDerivedState(x, chain_gs))
...
plotBranchHeatMap(tree, chains, "theta", burnin=0.3, nn=1000, pal, show.tip.label=FALSE, legend_ticks=seq(-2.5, 2.5, 0.5), edge.width=2)
...
pdf("../output/figures/regressions_tetrapods_gs_pruned_ch1-6.pdf", height=8, width=12)
...
for(i in (1:nrow(regressions))){
  ...
  if(i==1){
    ...
    barplot(setNames(sapply(Lpps, function(x) x[sumpars$sb[i-1],1]), c("chain1", "chain2", "chain3", "chain4", "chain5", "chain6")), ylim=c(0, 1), col="gray80", main="Posterior Probabilities")
  }
  ...
  print(descendents[[i]])
  expected <- regressions[i,1]+regressions[i,2]*pred[include,1]+regressions[i,3]*pred[include,2]+beta3*pred[include,3]
