...
chain <- readRDS(file="../output/runs/tetrapods/tetrapodsCombinedChains.rds")
plot(chain$lnL)

tree <- reorder(tree, "postorder")
#majorclades <- identifyBranches(tree, 5)
majorclades <- list(sb=c(1712, 1710, 846, 845, 1707), loc=rep(0,5))
...
mambirdesc <- c(cache$bdesc[[1707]], cache$bdesc[[845]])
endoMed <- median(chain$endo)
allbranches[mambirdesc,1] <- allbranches[mambirdesc,1]+endoMed
allbranches[c(1707,845),1] <- allbranches[c(1707,845),1]+endoMed
...
ticksInt <- seq(-2, 2, 0.5)
require(wesanderson)
pdf("~/repos/bmr/evolution2015/slopeintfig_onblack.pdf",width=12, height=10)
par(bg="black")
text.col <- "white"
cex.circle <- 2.5
palB <- wes_palette("FantasticFox")
par(mfrow=c(1,2), mar=c(0,0,0,1.5))
pal <- colorRampPalette(rev(c(palB[5],"#c0392b","#9b59b6", palB[3],"#2ecc71","#f1c40f", "yellow2","#27ae60")))
plot(tree, edge.color=colorRamp(allbranches[,1], pal, 100), cex=0.3,edge.width=1.5, show.tip.label=FALSE)
addColorBar(x=40, y=0, height=400, width=20, pal=pal, trait=allbranches[,1], ticks=seq(-5,2,1),adjx=2, cex.lab=2, text.col=text.col)
text(0,length(tree$tip.label), "Intercept", pos=4, cex=2.5, col=text.col)
nodelabels(text="III", node=tree$edge[1707,2], bg = "white",frame="circle",cex=cex.circle)
nodelabels(text="II", node=tree$edge[582,2], bg = "white",frame="circle",cex=cex.circle)
nodelabels(text="I", node=tree$edge[350,2], bg = "white",frame="circle",cex=cex.circle)
par(mar=c(0,1.5,0,0))
pal <- colorRampPalette(rev(c(palB[5],"#c0392b", palB[3],"#2ecc71","#f1c40f", "yellow2","#27ae60")))
plot(tree, edge.color=colorRamp(allbranches[,2], pal, 100), cex=0.3,edge.width=1.5, show.tip.label=FALSE, direction="leftwards")
addColorBar(x=454, y=0, height=400, width=20, pal=pal, trait=allbranches[,2], ticks=seq(0.65,0.8,length.out=5),adjx=5,cex.lab=2, text.col=text.col)
text(350,length(tree$tip.label), "Slope", pos=4,cex=2.5, col=text.col)
nodelabels(text="III", node=tree$edge[1707,2], bg = "white",frame="circle",cex=cex.circle)
nodelabels(text="II", node=tree$edge[582,2], bg = "white",frame="circle",cex=cex.circle)
nodelabels(text="I", node=tree$edge[350,2], bg = "white",frame="circle",cex=cex.circle)
dev.off()
...
### Figure 1B
i=5
palC <- wes_palette("Zissou")
par(bg="white")
cl1 <- "palegoldenrod" #palC[3]
cl2 <- "#5f54a0"#palC[1]
cl3 <- "#cdccd4" #palC[2]
cl4 <- "#5f54a0"#palC[1]
cl5 <- palC[4]
...
pdf("../doc/preproposal/figures/allometryfig_onblack.pdf",width=6.5, height=10)
{
par(mfrow=c(3,1), mar=c(1,1,1,1), bg="black", col.axis="white", col.lab="white", col.sub="white", col.main="white", col="white")
## Plethodontidae
imSal <- readPNG("../doc/preproposal/figures/Salamander.png")
...
node <- minorclades[1]
plot(lnMass, lnBMR, xaxt="n", yaxt="n",bty="l",lwd=lwd, pch=21, bg=makeTransparent(cl1, alph1), col=makeTransparent(cl1,0), xlab="", ylab="",xlim=Xlim, ylim=Ylim)
...
res2 <- cbind(thetas, beta1s)[!is.na(thetas),]
sq <- floor(seq(1, nrow(res2), length.out=200))
lapply(sq, function(x) abline(res2[x,1], res2[x, 2], lty=1, col=makeTransparent(cl3, alph),lwd=linelwd))
points(lnMass[id], lnBMR[id],  pch=21, bg=makeTransparent(cl2,100), col=makeTransparent(cl2,0))
ave <- apply(res2, 2, mean)
baseline <- rbind(res[[1]], res[[2]])
baselineave <- apply(baseline,2, median)
abline(ave[1], ave[2], col=cl4, lwd=4, lty=2)
abline(baselineave[1], baselineave[2], col=cl5, lwd=4, lty=2)
text(-1.5,Ylim[2]-0.5,labels="I. Plethodontidae",cex=4, pos=4)
rasterImage(colorizeRaster(imSal, cl2, alpha=1),-1,Ylim[2]-3.5,2,Ylim[2]-1.5, col="green")

## Serpentes
imSnake <- readPNG("../doc/preproposal/figures/Viperidae.png")
minorclades <- c("Plethodontidae"=344, "Serpentes"=578, "Colubridae"=456, "Boidae"=501, Passeriformes=822, "Chiroptera"=1618, Phrynosomatidae=528, Dendrobatidae=138)
node <- minorclades[3]
plot(lnMass, lnBMR, xaxt="n", yaxt="n",bty="l",lwd=lwd, pch=21, bg=makeTransparent(cl1, alph1), col=makeTransparent(cl1,0), xlab="", ylab="",xlim=Xlim, ylim=Ylim)
id <- tree$tip.label[cache$desc$tips[[tree$edge[node,2]]]]
...
abline(ave[1], ave[2], col=cl4, lwd=4, lty=2)
abline(baselineave[1], baselineave[2], col=cl5, lwd=4, lty=2)
text(-1.5,Ylim[2]-0.5,labels="II. Serpentes",cex=4, pos=4)
rasterImage(colorizeRaster(imSnake, cl2, alpha=1),-1,Ylim[2]-4.25,2,Ylim[2]-1.25, col="green")


###Mammals
imMamm <-  readPNG("../doc/preproposal/figures/Mammalia.png")
plot(lnMass, lnBMR, xaxt="n", yaxt="n",bty="l",lwd=lwd, pch=21, bg=makeTransparent(cl1, alph1), col=makeTransparent(cl1,0), xlab="", ylab="", xlim=Xlim, ylim=Ylim)
#axis(side = 1, lwd = lwd, col=1, col.axis=1)
#axis(side = 2, lwd = lwd,col=1, col.axis=1)
lapply(round(seq(1,nrow(res[[i]]), length.out=nline),0), function(x) abline(res[[i]][x,1], res[[i]][x, 2], lty=1, col=makeTransparent(cl3, alph),lwd=linelwd))
points(lnMass[majorcladetips[[i]]], lnBMR[majorcladetips[[i]]], pch=21, bg=makeTransparent(cl2,100), col=makeTransparent(cl2,0))
ave <- apply(res[[i]], 2, median)
baseline <- rbind(res[[1]], res[[2]])
baselineave <- apply(baseline,2, median)
abline(ave[1], ave[2], col=cl4, lwd=4, lty=2)
abline(baselineave[1], baselineave[2], col=cl5, lwd=4, lty=2)
text(-1.5,Ylim[2]-0.5,labels="III. Mammals",cex=4, pos=4)
rasterImage(colorizeRaster(imMamm, cl2, alpha=1),-1,Ylim[2]-4.25,2,Ylim[2]-1.25, col="white")
...
for(j in 1:length(clades)){
  ...
  majorcladecolors <- c("lightblue", "green", "orange", "purple", "red")
  plot(density(res2[[i]][,1]), type="n", xlim=c(-2, 2), ylim=c(0,7), main="Intercept", xlab="", ylab="")
