...
getStudyTrees <- function(studyTrees){  
  res <- lapply(which(studyTrees[,3]>2), function(x) .getStudyTrees(studyTrees[x,1], studyTrees[x,2], format="newick"))
  names(res) <- studyTrees[which(studyTrees[,3]>2),2]
  res <- res[which(!(sapply(res, is.null)))]
  summary <- data.frame(ntips = sapply(res, function(x) x$ntips),
                        matching = unlist(studyTrees[match(names(res), studyTrees[,2]),3],F,F),
                        timeUnits = sapply(res, function(x) x$branchLengthTimeUnit[[1]]),
                        description = sapply(res, function(x) x$branchLengthDescription[[1]]),
                        mode = sapply(sapply(res, function(x) x$branchLengthMode[[1]]), function(y) ifelse(is.null(y), "", y)))
...
sourceTreeCalibrations <- function(tree, taxalist){
  #tiplabs <- lapply(tree$tip.label, function(x) strsplit(x, "_ott"))
  #tiplabs <- sapply(tiplabs, function(x) x[[1]])
  #newTaxa <- tiplabs[!(tiplabs %in% taxalist)]
...
synthCalibrations <- function(focal_tree, source_trees){
  st_tips <- lapply(source_trees, function(x) x$tip.label)
  st_tips <- lapply(st_tips, function(x) gsub(" ", "_", x, fixed=TRUE))
  st_tips <- lapply(st_tips, function(x) gsub("'", "", x, fixed=TRUE))
  tips <- focal_tree$tip.label
  tips <- sapply(tips, function(x) strsplit(x, "_ott")[[1]][1])
  source_trees <- lapply(1:length(source_trees), function(x) {source_trees[[x]]$tip.label <- st_tips[[x]]; source_trees[[x]]})
  pruned_strees <- lapply(1:length(source_trees), function(x) drop.tip(source_trees[[x]], st_tips[[x]][!(st_tips[[x]] %in% tips)]))
  target <- focal_tree
  target$tip.label <- tips
  target$edge.length <- rep(1, nrow(target$edge))
  target <- chronopl(target, lambda=1)
  tax <- cbind(target$tip.label)
  rownames(tax) <- target$tip.label
  cals <- lapply(pruned_strees, function(x) congruify.phylo(x, target, tax, scale=NA))
  cals <- lapply(cals, function(x) x$calibrations)
  return(cals)
}
