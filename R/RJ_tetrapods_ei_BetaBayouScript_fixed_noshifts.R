...
#rm(list=ls(all=TRUE))
#setwd("~/repos/bmr/R/")
#args <- list("tetrapods_ei", 100000, "_delete_me")
...
custom.lik <- function(pars, cache, X, model="Custom"){
  ...
  X = X - pars$beta1[betaID]*pred[,1] - pars$endo*pred[,3] #- pars$beta3*pred[,3]
  cache$dat <- X
...
startpar <- list(k=0, ntheta=1, sb=numeric(0), loc=numeric(0), t2=numeric(0))
...
startpar <- startpar[c("alpha", "sig2", "endo", "k", "ntheta", "beta1", "theta", "sb", "loc", "t2")]
