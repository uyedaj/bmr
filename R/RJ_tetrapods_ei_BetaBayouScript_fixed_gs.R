...
#rm(list=ls(all=TRUE))
#setwd("~/repos/bmr/R/")
#args <- list("tetrapods_ei", 100000, "_delme20")
...
custom.lik <- function(pars, cache, X, model="Custom"){
  ...
  X = X - pars$beta1[betaID]*pred[,1] - pars$endo*pred[,4] - pars$beta3*pred[,3]
  cache$dat <- X
...
startpar$beta1 <- rnorm(length(startpar$sb)+1, 0.72, 0.2)
startpar$beta3 <- rnorm(1, 0, 0.1)
...
#pdf("genomesize.pdf", height=6, width=6)
#par(bg="black")
phenogram(tdgs$phy, setNames(tdgs$dat$lnGenSize, tdgs$phy$tip.label), fsize=0.5, spread.labels=FALSE, colors=setNames("black", 1))#$impute 
#dev.off()
...
startpar <- startpar[c("alpha", "sig2", "endo", "k", "ntheta", "beta3","pred.sig2", "pred.root", "beta1", "theta", "sb", "loc", "t2", "missing.pred")]
...
BetaBMR.monitor = function(i, lik, pr, pars, accept, accept.type, j){
  names <- c("gen", "lnL", "prior", "alpha","sig2", "rbeta1","beta3","endo", "k")
  ...
  cat(sprintf(string, i, lik, pr, pars$alpha, pars$sig2, pars$beta1[1], pars$beta3, pars$endo, pars$k), sprintf("%-8.2f", acceptratios),"\n", sep="")
}
...
model.BetaBMR <- list(moves = list(alpha=".multiplierProposal", sig2=".multiplierProposal", 
                      ...
                      control.weights = list(alpha=5, sig2=3, beta1=23, 
                                             #beta2=8, 
                                             beta3=4, 
                                             endo=2, 
                                             theta=23, slide=3, k=0,
...
chainnosal <- readRDS("../output/runs/tetrapods_ei/tetrapods_ei__nosal.chain.rds")

pdf("../evolution2015/beta3plot.pdf", height=7, width=7)
par(mfrow=c(1,1), mar=c(5,4,5,1), bg="black", ask=FALSE, col.axis="white", col.lab="white", col.main="white")
plot(density(chain$beta3, bw=0.02), type="n", col="white", lwd=3, xlim=c(-0.5, 0.5), ylim=c(0, 8), main="", yaxt="n", ylab="", xlab="")
box(col="white", lwd=2)
polygon(density(chain$beta3, bw=0.02), col=makeTransparent("white", 200))
#polygon(density(chainnosal$beta3, bw=0.05), col=makeTransparent("#e74c3c", 200))
abline(v=0, lty=2, col="white", lwd=5)
