...
rm(list=ls(all=TRUE))
setwd("~/repos/bmr/R/")
args <- list("tetrapods_gs", 50000, "fixed24_r1")
...
pred <- cbind(setNames(dat[['lnMass']], tree$tip.label),setNames(dat[['lnGenSize']], tree$tip.label))
...
missing <- which(is.na(cache$pred[,2]))
...
custom.lik <- function(pars, cache, X, model="Custom"){
  ...
  X <- cache$dat
  pred <- cache$pred
  pred[is.na(pred[,2]),2] <- pars$missing.pred
  ...
  X = X - pars$beta1[betaID]*pred[,1] - pars$beta2*pred[,2]
  cache$dat <- X
  dpars <- pars
...
sb <- c("Tetrapods"=1707, "Amniota"=1705,"Varanus_exanthematicus"=563, "Pseudoeurycea_brunnata"=277, "Sceloporus_variabilis"=515, "Sceloporus_occidentalis"=509, "Uta_stansburiana"=518, "Masticophis_flagellum"=443, "Typhlogobius_californiensis"=54, "Sebastolobus_altivelis"=85, "Lacertidae"=614, "Boidae"=501, "Hyla_arborea"=159, "Aves"=841, "Mammals"=1703, "Lichanura_trivirgata"=495, "Bunopus_tuberculatus"=655, "Fishes"=115, "Scaphiophidae"=261, "Euphlyctis"=227, "Labeo"=7, "Salmonidae"=48, "Cricetidae"=1222, "Ambystoma_mexicanum"=368)
k <- length(sb)
startpar <- list(alpha=0.1, sig2=3, beta1=rnorm(k+1, 0.7, 0.1), beta2=0.1, endo=2 , k=k, ntheta=k+1, theta=rnorm(k+1, 0, 1), sb=sb, loc=rep(0, k), t2=2:(k+1))
...
model.BetaBMR <- list(moves = list(alpha=".multiplierProposal", sig2=".multiplierProposal", beta1=".vectorMultiplier", beta2=".slidingWindowProposal", 
                                   endo=".slidingWindowProposal", theta=".adjustTheta", slide=".slide",  pred.sig2=".multiplierProposal", 
                                   pred.root=".slidingWindowProposal", missing.pred=".imputePredBM"),
                      control.weights = list("alpha"=4,"sig2"=2,"beta1"=8, "beta2"=4,"endo"=3, "theta"=8,"slide"=2,"k"=0, "pred.root"=2, "pred.sig2"=2, "missing.pred"=3),
                      D = list(alpha=1, sig2= 0.75, beta1=0.75, beta2=0.05, endo=0.25, theta=2, slide=1, pred.sig2=1, pred.root=2, missing.pred=1),
                      parorder = c("alpha", "sig2", "beta1", "beta2","endo", "pred.sig2", "pred.root", "missing.pred", "k", "ntheta", "theta"),
                      rjpars = numeric(0),
...
prior <- make.prior(tree, plot.prior = FALSE, dists=list(dalpha="dhalfcauchy", dsig2="dhalfcauchy",
                                                         dbeta1="dnorm",dbeta2="dnorm",dendo="dnorm",  dsb="fixed", dk="fixed", dtheta="dnorm",
                                                         dpred.sig2="dhalfcauchy", dpred.root="dnorm"), 
                    param=list(dalpha=list(scale=1), dsig2=list(scale=1), 
                               dbeta1=list(mean=0.7, sd=0.1), dbeta2=list(mean=0, sd=0.01),dendo=list(mean=0, sd=4),  dk="fixed", dsb="fixed", 
                               dtheta=list(mean=0, sd=4), dpred.sig2=list(scale=1), dpred.root=list(mean=1, sd=1)), fixed=list(sb=startpar$sb, k=startpar$k))
...
prior(startpar)
custom.lik(.imputePredBM(cache, startpar, 1, NULL)$pars, cache, cache$dat)$loglik
