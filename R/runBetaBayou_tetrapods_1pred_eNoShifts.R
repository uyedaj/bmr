...
rm(list=ls(all=TRUE))
setwd("~/repos/bmr/R/")
args <- list("tetrapods_gs", 100000, "_eNoShifts_r3")
...
custom.lik <- function(pars, cache, X, model="Custom"){
  ...
  X <- X- pred[,1]*pars$beta1
  cache$dat <- X
  dpars <- pars
  dpars$k <- 2
  dpars$ntheta <- 2
  dpars$sb <- c(841, 1703)
  dpars$theta <- c(dpars$theta, dpars$theta+dpars$endo)
  dpars$loc <- cache$edge.length[dpars$sb]/2
  dpars$t2 <- c(2,2)
...
k <- length(sb)
startpar <- list(alpha=0.1, sig2=3, beta1=0.7, endo=3 , k=0, ntheta=1, theta=-1, sb=numeric(0),loc=numeric(0), t2=numeric(0))
...
BetaBMR.monitor = function(i, lik, pr, pars, accept, accept.type, j){
  ...
  cat(sprintf(string, i, lik, pr, pars$alpha, pars$sig2, pars$beta1, pars$endo, pars$k), sprintf("%-8.2f", acceptratios),"\n", sep="")
}

## We're going to define a custom model with variable slopes (beta1)
model.BetaBMR <- list(moves = list(alpha=".multiplierProposal", sig2=".multiplierProposal", beta1=".vectorMultiplier", endo=".slidingWindowProposal", theta=".adjustTheta"),
                      control.weights = list("alpha"=5,"sig2"=2,"beta1"=5,"endo"=5, "theta"=5, "k"=0),
                      D = list(alpha=0.5, sig2= 0.5, beta1=0.1, endo=0.25, theta=2, slide=1),
...
prior <- make.prior(tree, plot.prior = TRUE, dists=list(dalpha="dhalfcauchy", dsig2="dhalfcauchy",
                                     dbeta1="dnorm", dendo="dnorm", dsb="fixed", dk="fixed",dloc="fixed", dtheta="dnorm"), 
                          param=list(dalpha=list(scale=1), dsig2=list(scale=1), 
                               dbeta1=list(mean=0.7, sd=0.1), dendo=list(mean=0, sd=4), dk="fixed", dsb="fixed",dloc="fixed", 
                               dtheta=list(mean=0, sd=4)), model="ffancova", fixed=list(sb=startpar$sb, k=startpar$k, loc=numeric(0)))
