...
#rm(list=ls(all=TRUE))
#setwd("~/repos/bmr/R/")
#args <- list("kolokotrones", 10000, "_kokonq_iT4")
...
custom.lik <- function(pars, cache, X, model="Custom"){
  ...
  X = X - pars$beta1[betaID]*pred[,1] - pars$beta3*pred[,3]#- pars$endo*pred[,3] #- pars$beta3*pred[,3]
  cache$dat <- X
...
startpars_gen <- function(sb, k) {
  ...
  startpar <- list(alpha=5, sig2=5, beta1=rnorm(k+1, 0.7, 0.05), beta3=rnorm(1, -1103, 801), k=k, ntheta=k+1, theta=rnorm(k+1, -2.5, 1), sb=sb, loc=rep(0, k), t2=2:(k+1))
  return(startpar)
}
...
startpar <- startpar[c("alpha", "sig2", "k", "ntheta", "beta3","pred.sig2", "pred.root", "missing.pred", "beta1", "theta", "sb", "loc", "t2")]
...
BetaBMR.monitor = function(i, lik, pr, pars, accept, accept.type, j){
  names <- c("gen", "lnL", "prior", "alpha","sig2", "rbeta1", "beta3", "k")
  ...
  cat(sprintf(string, i, lik, pr, pars$alpha, pars$sig2, pars$beta1[1],pars$beta3 , pars$k), sprintf("%-8.2f", acceptratios),"\n", sep="")
}
...
prior <- make.prior(tree, plot.prior = FALSE, 
                    ...
                    param=list(dalpha=list(scale=1), dsig2=list(scale=1), dbeta1=list(mean=0.7, sd=0.3), 
                               #dbeta2=list(mean=0, sd=0.01), 
                               dbeta3=list(mean=-1000, sd=1000), 
