...
majorclades <- list(sb=c(1708, 1706, 842, 841, 1703), loc=rep(0,5))
...
require(wesanderson)
pdf("../doc/preproposal/figures/slopeintfig_onblack.pdf",width=12, height=10)
par(bg="black")
text.col <- "white"
cex.circle <- 2.5
palB <- wes_palette("FantasticFox")
par(mfrow=c(1,2), mar=c(0,0,0,1.5))
pal <- colorRampPalette(rev(c(palB[5],palB[5], palB[5],palB[5],palB[4], palB[4],palB[3], palB[2], "yellow2")))
plot(tree, edge.color=colorRamp(allbranches[,1], pal, 100), cex=0.3,edge.width=1.5, show.tip.label=FALSE)
addColorBar(x=40, y=0, height=400, width=20, pal=pal, trait=allbranches[,1], ticks=seq(-2,2,1),adjx=2, cex.lab=2, text.col=text.col)
text(0,length(tree$tip.label), "Intercept", pos=4, cex=2.5, col=text.col)
nodelabels(text="III", node=tree$edge[1703,2], bg = "white",frame="circle",cex=cex.circle)
nodelabels(text="II", node=tree$edge[578,2], bg = "white",frame="circle",cex=cex.circle)
nodelabels(text="I", node=tree$edge[344,2], bg = "white",frame="circle",cex=cex.circle)
par(mar=c(0,1.5,0,0))
plot(tree, edge.color=colorRamp(allbranches[,2], pal, 100), cex=0.3,edge.width=1.5, show.tip.label=FALSE, direction="leftwards")
addColorBar(x=454, y=0, height=400, width=20, pal=pal, trait=allbranches[,2], ticks=seq(0.68,0.8,length.out=5),adjx=5,cex.lab=2, text.col=text.col)
