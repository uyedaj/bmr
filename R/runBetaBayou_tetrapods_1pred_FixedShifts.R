...
#rm(list=ls(all=TRUE))
#setwd("~/repos/bmr/R/")
#args <- list("tetrapods_gs", 10000, "_fixed_r4")
...
k <- length(sb)
startpar <- list(alpha=0.1, sig2=3, beta1=rnorm(k+1, 0.7, 0.1), k=k, ntheta=k+1, theta=rnorm(k+1, 0, 1), sb=sb, loc=rep(0, k), t2=2:(k+1))
...
model.BetaBMR <- list(moves = list(alpha=".multiplierProposal", sig2=".multiplierProposal", beta1=".vectorMultiplier", theta=".adjustTheta", slide=".slide"),
                      control.weights = list("alpha"=5,"sig2"=2,"beta1"=10, "theta"=10,"slide"=2,"k"=0),
                      D = list(alpha=0.5, sig2= 0.5, beta1=0.75, theta=2, slide=1),
...
prior <- make.prior(tree, plot.prior = TRUE, dists=list(dalpha="dhalfcauchy", dsig2="dhalfcauchy",
                                     dbeta1="dnorm", dsb="fixed", dk="fixed", dtheta="dnorm"), 
                          param=list(dalpha=list(scale=1), dsig2=list(scale=1), 
                               dbeta1=list(mean=0.7, sd=0.1), dk="fixed", dsb="fixed", 
