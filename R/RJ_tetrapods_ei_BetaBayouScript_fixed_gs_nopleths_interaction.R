...
#rm(list=ls(all=TRUE))
#setwd("~/repos/bmr/R/")
#args <- list("tetrapods_ei", 50000, "_nopleth_xM_r2")
...
custom.lik <- function(pars, cache, X, model="Custom"){
  ...
  X = X - pars$beta1[betaID]*pred[,1] - pars$endo*pred[,4] - pars$beta3*pred[,3] - pars$beta4*pred[,3]*pred[,1]
  cache$dat <- X
...
startpar$sb <- startpar$sb[!(startpar$sb %in% c(350))]
startpar$k <- length(startpar$sb)
startpar$ntheta <- length(startpar$sb)+1
startpar$loc <- startpar$loc[1:length(startpar$sb)]
startpar$t2 <- 2:(length(startpar$sb)+1)
...
startpar <- startpar[c("alpha", "sig2", "endo", "k", "ntheta", "beta3","beta4","pred.sig2", "pred.root", "beta1", "theta", "sb", "loc", "t2", "missing.pred")]
...
BetaBMR.monitor = function(i, lik, pr, pars, accept, accept.type, j){
  names <- c("gen", "lnL", "prior", "alpha","sig2", "rbeta1","beta3","beta4","endo", "k")
  string <- "%-8i%-8.2f%-8.2f%-8.2f%-8.2f%-8.2f%-8.2f%-8.2f%-8.2f%-8i"
  acceptratios <- tapply(accept, accept.type, mean)
  names <- c(names, names(acceptratios))
  if(j==0){
    cat(sprintf("%-7.7s", names), "\n", sep=" ")                           
  }
  cat(sprintf(string, i, lik, pr, pars$alpha, pars$sig2, pars$beta1[1], pars$beta3, pars$beta4, pars$endo, pars$k), sprintf("%-8.2f", acceptratios),"\n", sep="")
}
...
model.BetaBMR <- list(moves = list(alpha=".multiplierProposal", sig2=".multiplierProposal", 
                      ...
                      control.weights = list(alpha=5, sig2=3, beta1=23, 
                                             ...
                                             beta4=4,
                                             endo=2, 
                                             theta=23, slide=0, k=0,
                                             #pred.sig2=1, pred.root=1, 
                                             missing.pred=3 #$impute
                                             ),
                      D = list(alpha=0.7, sig2= 0.5, beta1=0.75, 
                               #beta2=0.05, 
                               beta3=0.2, 
                               beta4=0.05,
                               endo=0.5, 
