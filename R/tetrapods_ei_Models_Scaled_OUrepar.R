...
lnBMR <- setNames(dat[['lnBMR']], tree$tip.label)
lnMass <- setNames(dat[['lnMass']], tree$tip.label)
lnMass2 <- setNames(dat[['lnMass']]^2, tree$tip.label)
lnGS <- setNames(dat[['lnGenSize']], tree$tip.label)
#.pred <- cbind(setNames(dat[['lnMass']], tree$tip.label), setNames(dat[['lnMass']]^2, tree$tip.label), setNames(dat[['lnGenSize']], tree$tip.label))
#colnames(.pred) <- c("lnMass", "lnMass2", "lnGS")
...
param.Vy <- list(min=0.01, max=)
param.beta_lnMass <- list(meandhal=0.7*scalePars$lnMass$`scaled:scale`, sd=0.15*scalePars$lnMass$`scaled:scale`)
param.beta_lnMass2 <- list(mean=0, sd=1)
param.beta_lnGS <- list(mean=0, sd=1)
param.beta_lnGSxlnMass <- list(mean=0, sd=1)
...
param.theta <- list(mean=-2+0.70*scalePars$lnMass$`scaled:center`, sd=1.75)
...
## Proposal widths
D.XXX <- function(nrj) list(halflife=c(0.03, 0.7), Vy= c(0.03, 0.7), beta_lnMass=c(0.5,2), beta_lnMass2=0.004, beta_lnGS=0.2, beta_lnMassxlnGS=0.05, beta_endo=0.5, k=2*c(3.5, 0.7), theta=c(1.5, 10), slide=1, missing.pred=1)
D.1XX <- function(nrj) list(halflife=c(0.03, 0.7), Vy= c(0.03, 0.7), beta_lnMass=c(0.1, 1), beta_lnMass2=0.004, beta_lnGS=0.2, beta_lnMassxlnGS=0.05, beta_endo=0.5, k=rep(1,nrj), theta=c(0.15,5), slide=1, missing.pred=1)
D.XNX <- function(nrj) list(halflife=c(0.03, 0.7), Vy= c(0.03, 0.7), beta_lnMass=c(0.5,2), beta_lnMass2=0.004, beta_lnGS=0.2, beta_lnMassxlnGS=0.05, beta_endo=0.5, k=rep(1,nrj), theta=c(1.5, 10), slide=1, missing.pred=1)
...
prior.11000 <- make.prior(tree, plot.prior = FALSE, model="OUrepar",
                    dists=list(dhalflife="dlnorm", dVy="dhalfcauchy", dbeta_lnMass="dnorm",
                               #dbeta_TempK="dnorm",
                               dbeta_endo="dnorm",
                               dsb="fixed", dk="fixed", dtheta="dnorm"), 
                    param=list(dhalflife=param.halflife, dVy=param.Vy, dbeta_lnMass=param.beta_lnMass,
...
model.11000 <- makeBayouModel(lnBMR ~ lnMass + endo, rjpars = numeric(0), cache, prior.11000, impute = NULL, D=D.1XX(1), model="OUrepar")
...
prior.11100 <- make.prior(tree, plot.prior=FALSE, model="OUrepar", 
                          dists=list(dhalflife="dlnorm", dVy="dhalfcauchy", dbeta_lnMass="dnorm",
                                     dbeta_lnMass2="dnorm",
                                     #dbeta_TempK="dnorm",
                                     dbeta_endo="dnorm",
...
model.11100 <- makeBayouModel(lnBMR ~ lnMass + lnMass2 + endo, rjpars = numeric(0), cache, prior.11100, impute = NULL, D=D.1XX(1), model="OUrepar")
...
prior.mamm11100 <- make.prior(mammtree, plot.prior=FALSE, model="OUrepar", 
                          dists=list(dhalflife="dlnorm", dVy="dhalfcauchy", dbeta_lnMass="dnorm",
                                     dbeta_lnMass2="dnorm",
                                     #dbeta_TempK="dnorm",
                                     #dbeta_endo="dnorm",
                                     dsb="fixed", dk="fixed", dtheta="dnorm"), 
                          param=list(dhalflife=param.halflife, dVy=param.Vy, 
...
model.mamm11100 <- makeBayouModel(lnBMR ~ lnMass + lnMass2, rjpars = numeric(0), mammcache, prior.mamm11100, impute = NULL, D=D.1XX(1), model="OUrepar")
...
prior.mamm11000 <- make.prior(mammtree, plot.prior=FALSE, model="OUrepar", 
                              dists=list(dhalflife="dlnorm", dVy="dhalfcauchy", dbeta_lnMass="dnorm",
                                         #dbeta_TempK="dnorm",
                                         #dbeta_endo="fixed",
                                         dsb="fixed", dk="fixed", dtheta="dnorm"), 
...
model.mamm11000 <- makeBayouModel(lnBMR ~ lnMass, rjpars = numeric(0), mammcache, prior.mamm11000, impute = NULL, D=D.1XX(1), model="OUrepar")
...
prior.N1100 <- make.prior(tree, plot.prior=FALSE, model="OUrepar", 
                        dists=list(dhalflife="dlnorm", dVy="dhalfcauchy", dbeta_lnMass="dnorm",
                                              dbeta_lnMass2="dnorm",
                                              #dbeta_TempK="dnorm",
                                              dbeta_endo="dnorm",
...
model.N1100 <- makeBayouModel(lnBMR ~ lnMass + lnMass2 + endo, rjpars = c("theta"), cache, prior.N1100, impute = NULL, D=D.XXX(1), model="OUrepar")
prior.N1100(model.N1100$startpar)
model.N1100$model$lik.fn(model.N1100$startpar, cache, cache$dat)$loglik

## NN100
prior.NN100 <- make.prior(tree, plot.prior=FALSE, model="OUrepar", 
                          dists=list(dhalflife="dlnorm", dVy="dhalfcauchy", dbeta_lnMass="dnorm",
                                     dbeta_lnMass2="dnorm",
                                     #dbeta_TempK="dnorm",
                                     dbeta_endo="dnorm",
...
model.NN100 <- makeBayouModel(lnBMR ~ lnMass + lnMass2 + endo, rjpars = c("theta", "lnMass"), cache, prior.NN100, impute = NULL, D=D.XXX(1), model="OUrepar")
...
prior.11010 <- make.prior(tree, plot.prior=FALSE, model="OUrepar", 
                          dists=list(dhalflife="dlnorm", dVy="dhalfcauchy", dbeta_lnMass="dnorm",
                                     #dbeta_lnMass2="dnorm",
                                     dbeta_lnGS="dnorm",
                                     #dbeta_TempK="dnorm",
...
model.11010 <- makeBayouModel(lnBMR ~ lnMass + lnGS + endo, rjpars = numeric(0), cache, prior.11010, impute = c("lnGS"), D=D.1XX(1), model="OUrepar")
...
prior.11011 <- make.prior(tree, plot.prior=FALSE, model="OUrepar", 
                          dists=list(dhalflife="dlnorm", dVy="dhalfcauchy", dbeta_lnMass="dnorm",
                                     #dbeta_lnMass2="dnorm",
                                     dbeta_lnGS="dnorm",
...
model.11011 <- makeBayouModel(lnBMR ~ lnMass + lnGS + endo + lnGS*lnMass, rjpars = numeric(0), cache, prior.11011, impute = c("lnGS"), D=D.1XX(1), model="OUrepar")
...
prior.N1010 <- make.prior(tree, plot.prior=FALSE, model="OUrepar", 
                          dists=list(dhalflife="dlnorm", dVy="dhalfcauchy", dbeta_lnMass="dnorm",
                                     #dbeta_lnMass2="dnorm",
                                     dbeta_lnGS="dnorm",
                                     #dbeta_TempK="dnorm",
...
model.N1010 <- makeBayouModel(lnBMR ~ lnMass + lnGS + endo, rjpars = c("theta"), cache, prior.N1010, impute = c("lnGS"), D=D.XXX(1), model="OUrepar")
...
prior.NN010 <- make.prior(tree, plot.prior=FALSE, model="OUrepar", 
                          dists=list(dhalflife="dlnorm", dVy="dhalfcauchy", dbeta_lnMass="dnorm",
                                     #dbeta_lnMass2="dnorm",
                                     dbeta_lnGS="dnorm",
                                     #dbeta_TempK="dnorm",
...
model.NN010 <- makeBayouModel(lnBMR ~ lnMass + lnGS + endo, rjpars = c("theta", "lnMass"), cache, prior.NN010, impute = c("lnGS"), D=D.XXX(2), model="OUrepar")
...
prior.N1011 <- make.prior(tree, plot.prior=FALSE, model="OUrepar", 
                          dists=list(dhalflife="dlnorm", dVy="dhalfcauchy", dbeta_lnMass="dnorm",
                                     #dbeta_lnMass2="dnorm",
                                     dbeta_lnGS="dnorm",
                                     dbeta_lnMassxlnGS="dnorm",
...
model.N1011 <- makeBayouModel(lnBMR ~ lnMass + lnGS + endo + lnGS*lnMass, rjpars = c("theta"), cache, prior.N1011, impute = c("lnGS"), D=D.XXX(1), model="OUrepar")
...
prior.NN011 <- make.prior(tree, plot.prior=FALSE, model="OUrepar", 
                          dists=list(dhalflife="dlnorm", dVy="dhalfcauchy", dbeta_lnMass="dnorm",
                                     #dbeta_lnMass2="dnorm",
                                     dbeta_lnGS="dnorm",
                                     dbeta_lnMassxlnGS="dnorm",
...
model.NN011 <- makeBayouModel(lnBMR ~ lnMass + lnGS + endo + lnGS*lnMass, rjpars = c("theta", "lnMass"), cache, prior.NN011, impute = c("lnGS"), D=D.XXX(2), model="OUrepar")
prior.NN011(model.NN011$startpar)
model.NN011$model$lik.fn(model.NN011$startpar, cache, cache$dat)$loglik

### No Salamanders
## Separate intercepts, genome size
prior.S1010 <- make.prior(tree, plot.prior=FALSE, model="OUrepar", 
                          dists=list(dhalflife="dlnorm", dVy="dhalfcauchy", dbeta_lnMass="dnorm",
                                     #dbeta_lnMass2="dnorm",
                                     dbeta_lnGS="dnorm",
                                     #dbeta_TempK="dnorm",
...
model.S1010 <- makeBayouModel(lnBMR ~ lnMass + lnGS + endo, rjpars = c("theta"), cache, prior.S1010, impute = c("lnGS"), D=D.XXX(1), model="OUrepar")
prior.S1010(model.S1010$startpar)
model.S1010$model$lik.fn(model.S1010$startpar, cache, cache$dat)$loglik

## Separate intercepts, separate slopes, genome size
prior.SS010 <- make.prior(tree, plot.prior=FALSE, model="OUrepar", 
                          dists=list(dhalflife="dlnorm", dVy="dhalfcauchy", dbeta_lnMass="dnorm",
                                     #dbeta_lnMass2="dnorm",
                                     dbeta_lnGS="dnorm",
                                     #dbeta_TempK="dnorm",
...
model.SS010 <- makeBayouModel(lnBMR ~ lnMass + lnGS + endo, rjpars = c("theta", "lnMass"), cache, prior.SS010, impute = c("lnGS"), D=D.XXX(2), model="OUrepar")
prior.SS010(model.SS010$startpar)
model.SS010$model$lik.fn(model.SS010$startpar, cache, cache$dat)$loglik

## Separate intercepts, genome size + interaction
prior.S1011 <- make.prior(tree, plot.prior=FALSE, model="OUrepar", 
                          dists=list(dhalflife="dlnorm", dVy="dhalfcauchy", dbeta_lnMass="dnorm",
                                     #dbeta_lnMass2="dnorm",
                                     dbeta_lnGS="dnorm",
                                     dbeta_lnMassxlnGS="dnorm",
...
model.S1011 <- makeBayouModel(lnBMR ~ lnMass + lnGS + endo + lnGS*lnMass, rjpars = c("theta"), cache, prior.S1011, impute = c("lnGS"), D=D.XXX(1), model="OUrepar")
prior.S1011(model.S1011$startpar)
model.S1011$model$lik.fn(model.S1011$startpar, cache, cache$dat)$loglik

## Separate intercepts, separate slopes, genome size + interaction
prior.SS011 <- make.prior(tree, plot.prior=FALSE, model="OUrepar", 
                          dists=list(dhalflife="dlnorm", dVy="dhalfcauchy", dbeta_lnMass="dnorm",
                                     #dbeta_lnMass2="dnorm",
                                     dbeta_lnGS="dnorm",
                                     dbeta_lnMassxlnGS="dnorm",
...
model.SS011 <- makeBayouModel(lnBMR ~ lnMass + lnGS + endo + lnGS*lnMass, rjpars = c("theta", "lnMass"), cache, prior.SS011, impute = c("lnGS"), D=D.XXX(2), model="OUrepar")
prior.SS011(model.SS011$startpar)
model.SS011$model$lik.fn(model.SS011$startpar, cache, cache$dat)$loglik

### No Plethodon
## Separate intercepts, genome size
prior.P1010 <- make.prior(tree, plot.prior=FALSE, model="OUrepar", 
                          dists=list(dhalflife="dlnorm", dVy="dhalfcauchy", dbeta_lnMass="dnorm",
                                     #dbeta_lnMass2="dnorm",
                                     dbeta_lnGS="dnorm",
                                     #dbeta_TempK="dnorm",
...
model.P1010 <- makeBayouModel(lnBMR ~ lnMass + lnGS + endo, rjpars = c("theta"), cache, prior.P1010, impute = c("lnGS"), D=D.XXX(1), model="OUrepar")
prior.P1010(model.P1010$startpar)
model.P1010$model$lik.fn(model.P1010$startpar, cache, cache$dat)$loglik

## Separate intercepts, separate slopes, genome size
prior.PP010 <- make.prior(tree, plot.prior=FALSE, model="OUrepar", 
                          dists=list(dhalflife="dlnorm", dVy="dhalfcauchy", dbeta_lnMass="dnorm",
                                     #dbeta_lnMass2="dnorm",
                                     dbeta_lnGS="dnorm",
                                     #dbeta_TempK="dnorm",
...
model.PP010 <- makeBayouModel(lnBMR ~ lnMass + lnGS + endo, rjpars = c("theta", "lnMass"), cache, prior.PP010, impute = c("lnGS"), D=D.XXX(2), model="OUrepar")
prior.PP010(model.PP010$startpar)
model.PP010$model$lik.fn(model.PP010$startpar, cache, cache$dat)$loglik

## Separate intercepts, genome size + interaction
prior.P1011 <- make.prior(tree, plot.prior=FALSE, model="OUrepar", 
                          dists=list(dhalflife="dlnorm", dVy="dhalfcauchy", dbeta_lnMass="dnorm",
                                     #dbeta_lnMass2="dnorm",
                                     dbeta_lnGS="dnorm",
                                     dbeta_lnMassxlnGS="dnorm",
...
model.P1011 <- makeBayouModel(lnBMR ~ lnMass + lnGS + endo + lnGS*lnMass, rjpars = c("theta"), cache, prior.P1011, impute = c("lnGS"), D=D.XXX(1), model="OUrepar")
prior.P1011(model.P1011$startpar)
model.P1011$model$lik.fn(model.P1011$startpar, cache, cache$dat)$loglik

## Separate intercepts, separate slopes, genome size + interaction
prior.PP011 <- make.prior(tree, plot.prior=FALSE, model="OUrepar", 
                          dists=list(dhalflife="dlnorm", dVy="dhalfcauchy", dbeta_lnMass="dnorm",
                                     #dbeta_lnMass2="dnorm",
                                     dbeta_lnGS="dnorm",
                                     dbeta_lnMassxlnGS="dnorm",
...
model.PP011 <- makeBayouModel(lnBMR ~ lnMass + lnGS + endo + lnGS*lnMass, rjpars = c("theta", "lnMass"), cache, prior.PP011, impute = c("lnGS"), D=D.XXX(2), model="OUrepar")
prior.PP011(model.PP011$startpar)
model.PP011$model$lik.fn(model.PP011$startpar, cache, cache$dat)$loglik

## NN000
prior.NN000 <- make.prior(tree, plot.prior=FALSE, model="OUrepar", 
                          dists=list(dhalflife="dlnorm", dVy="dhalfcauchy", dbeta_lnMass="dnorm",
                                     #dbeta_lnMass2="dnorm",
                                     #dbeta_lnGS="dnorm",
                                     #dbeta_TempK="dnorm",
...
model.NN000 <- makeBayouModel(lnBMR ~ lnMass + endo, rjpars = c("theta", "lnMass"), cache, prior.NN000, D=D.XXX(2), model="OUrepar")
prior.NN000(model.NN000$startpar)
model.NN000$model$lik.fn(model.NN000$startpar, cache, cache$dat)$loglik

## RR000
prior.RR000 <- make.prior(tree, plot.prior=FALSE, model="OUrepar", 
                          dists=list(dhalflife="dlnorm", dVy="dhalfcauchy", dbeta_lnMass="dnorm",
                                     #dbeta_lnMass2="dnorm",
                                     #dbeta_lnGS="dnorm",
                                     #dbeta_TempK="dnorm",
...
model.RR000 <- makeBayouModel(lnBMR ~ lnMass + endo, rjpars = c("theta", "lnMass"), cache, prior.RR000, D=D.XXX(2), model="OUrepar")
