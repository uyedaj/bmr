...
#rm(list=ls(all=TRUE))
#setwd("~/repos/bmr/R/")
#args <- list("tetrapods_ei", 50000, "_delme19")
...
custom.lik <- function(pars, cache, X, model="Custom"){
  ...
  X = X - pars$beta1*pred[,1] - pars$endo*pred[,4] - pars$beta3*pred[,3]
  cache$dat <- X
...
BetaBMR.monitor = function(i, lik, pr, pars, accept, accept.type, j){
  ...
  cat(sprintf(string, i, lik, pr, pars$alpha, pars$sig2, pars$beta1, pars$beta3, pars$endo, pars$k), sprintf("%-8.2f", acceptratios),"\n", sep="")
}
